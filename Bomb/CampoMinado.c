...
int main() { //Função principal
    ...
    while (!WindowShouldClose()) { //Loop principal do jogo
        ...
        for (int y = 0; y < TAMANHO_TABULEIRO; y++) { //Percorre todas as células do tabuleiro
            for (int x = 0; x < TAMANHO_TABULEIRO; x++) {
                ...
                if (mouseDentroTabuleiro && mouseX == x && mouseY == y) { //Mostra as bombas ao redor do mouse
...
void RevelarCelula(EstadoJogo *jogo, int x, int y, int numeroBombas, Sound explosao) {
    ...
    if (!jogo->tabuleiro[y][x].revelado) {
        ...
        if (jogo->tabuleiro[y][x].temBomba) { //Casa com bomba
            PlaySound(explosao);
            jogo->vidas--;
            if (jogo->vidas <= 0) {
